...
const Terminal = forwardRef(function Terminal({folder, onOutput}, ref) {
    ...
    // ── Core PTY starter — accepts an explicit folderPath ──────────────────────
    const startTerminalInFolder = useCallback(async (folderPath, {autoRunCodex = false, useResume = false} = {}) => {
        if (!folderPath || isRunningRef.current) return
        ...
        const result = await window.api.ptyStart(folderPath)
        if (result.error) {
            isRunningRef.current = false
            setIsRunning(false)
            syncCodexState('idle')
    ...
    }, [onOutput, focusTerminal, syncCodexState, launchCodex, resumeCodex])

    // Convenience wrapper that uses the current folder prop
    const startTerminal = useCallback((opts) => {
        return startTerminalInFolder(folder, opts)
    }, [folder, startTerminalInFolder])

    // ── Expose changeFolder to parent via ref ──────────────────────────────────
    useImperativeHandle(ref, () => ({
        changeFolder: async (newFolder) => {
            // 1. Gracefully stop Codex if running
            ...
            // 3. Announce the switch in the terminal
            xtermRef.current?.clear()
            xtermRef.current?.writeln(`\x1b[38;5;245m# Switched folder → ${newFolder}\x1b[0m`)
            // 4. Start fresh PTY in new folder and resume the same Codex session
            await startTerminalInFolder(newFolder, {autoRunCodex: true, useResume: true})
        }
    }), [syncCodexState, startTerminalInFolder])
